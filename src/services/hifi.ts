...
// Parse TIDAL endpoints from environment variable
// Format: name|url|quality,name2|url2|quality2
function parseTidalEndpoints(): { name: string; url: string; maxQuality: string }[] {
    const envValue = process.env.EXPO_PUBLIC_TIDAL_ENDPOINTS || '';
    if (!envValue) return [];

    return envValue.split(',').map((entry: string) => {
        const [name, url, maxQuality] = entry.split('|');
        return { name: name || '', url: url || '', maxQuality: maxQuality || 'LOSSLESS' };
    }).filter((e: { url: string }) => e.url);
}

// Parse Qobuz stream endpoints from environment variable
// Format: name|url|paramName|quality,name2|url2|paramName2|quality2
function parseQobuzStreamEndpoints(): { name: string; url: string; paramName: string; quality?: string }[] {
    const envValue = process.env.EXPO_PUBLIC_QOBUZ_STREAM_ENDPOINTS || '';
    if (!envValue) return [];

    return envValue.split(',').map((entry: string) => {
        const [name, url, paramName, quality] = entry.split('|');
        return {
            name: name || '',
            url: url || '',
            paramName: paramName || 'track_id',
            ...(quality ? { quality } : {})
        };
    }).filter((e: { url: string }) => e.url);
}

// Public TIDAL API endpoints - loaded from environment
const TIDAL_API_ENDPOINTS = parseTidalEndpoints();
...
// Qobuz API endpoints - loaded from environment
const QOBUZ_API = {
    search: process.env.EXPO_PUBLIC_QOBUZ_SEARCH || '',
    album: process.env.EXPO_PUBLIC_QOBUZ_ALBUM || '',
    playlist: process.env.EXPO_PUBLIC_QOBUZ_PLAYLIST || '',
    stream: parseQobuzStreamEndpoints(),
};
